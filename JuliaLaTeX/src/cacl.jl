macro byRow(pair::Expr)
    ...
    key  = if typeof(lambda.args[1]) == Symbol
        
        Expr(:vect, QuoteNode(lambda.args[1]))
    else
        
         Expr(:vect, QuoteNode.(lambda.args[1].args)...)
    end
...
function inlineConstAndVars(
    ex::Expr,
    vars::Union{Dict{Symbol,Union{<:Number,Expr,Symbol}},Nothing}=nothing; stacklevel::Int=0,
